...
zh_font = FontProperties(
    fname="/usr/share/fonts/oprntype/noto/NotoSansCJK-Regular.ttc"
)  # 按你机器字体路径调整
...
def train_step(input_batch, target_batch,
    ...
    # (L, B, H)
    encoder_outputs = torch.zeros(max_length, batch_size,
    ...
    # ---------- Decoder ----------
    decoder_input = torch.full((batch_size,),
                               SOS_token,
                               dtype=torch.long,
                               device=device)
    ...
    if use_teacher_forcing:
        for di in range(target_length):
            ...
            loss += criterion(decoder_output, target_step)
